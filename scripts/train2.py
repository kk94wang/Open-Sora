...
class DatasetFromCSV2(DatasetFromCSV):
    ...
    def __init__(
        ...
        super().__init__(csv_path, num_frames, frame_interval, transform, root)
        self.csv_path2 = csv_path2
        with open(csv_path2, "r") as f:
            reader = csv.reader(f)
            self.samples2 = list(reader)

        self.sks_length = len(self.samples2)

    def getitem(self, index):
        # if ramdom.random() > 0.25, choose self.samples, else choose from self.samples2
        if random.random() > 0.25:
            sample = self.samples[index]
        else:
            sample = self.samples2[index % self.sks_length]
        path = sample[0]
        if self.root:
...
def main():
    ...
    if not coordinator.is_master():
        ...
        if cfg.wandb:
            wandb.init(project="jackie_chan finetune", name=exp_name, config=cfg._cfg_dict)
